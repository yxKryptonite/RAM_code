...
def get_distance_imd(feature1, feature2, mask1, mask2, RESOLUTION=64):
    # feature1, feature2: 1,c,h',w'
    mask1, mask2 = torch.from_numpy(mask1[...,0]).cuda(), torch.from_numpy(mask2[...,0]).cuda() # h,w
    patch_size = RESOLUTION
    resized_src_mask = F.interpolate(mask1.unsqueeze(0).unsqueeze(0), size=(patch_size, patch_size), mode='nearest').squeeze().cuda() # h,w -> 1,patch_size,patch_size
    resized_tgt_mask = F.interpolate(mask2.unsqueeze(0).unsqueeze(0), size=(patch_size, patch_size), mode='nearest').squeeze().cuda() # h,w -> 1,patch_size,patch_size
    src_feature_upsampled = F.interpolate(feature1, size=(patch_size, patch_size), mode='bilinear').squeeze() # c,patch_size,patch_size
    tgt_feature_upsampled = F.interpolate(feature2, size=(patch_size, patch_size), mode='bilinear').squeeze() # c,patch_size,patch_size
    # mask the feature
    src_feature_upsampled = src_feature_upsampled * resized_src_mask.repeat(src_feature_upsampled.shape[0],1,1)
    tgt_feature_upsampled = tgt_feature_upsampled * resized_tgt_mask.repeat(src_feature_upsampled.shape[0],1,1)
    # Set the masked area to a very small number
    ...
    tgt_feature_upsampled[tgt_feature_upsampled == 0] = -100000
    # Calculate the cosine similarity between src_feature and tgt_feature
    src_features_2d=src_feature_upsampled.reshape(src_feature_upsampled.shape[0],-1).permute(1,0) # patch_size*patch_size, c
    tgt_features_2d=tgt_feature_upsampled.reshape(tgt_feature_upsampled.shape[0],-1).permute(1,0) # patch_size*patch_size, c
...
def get_distance_bbnn(feature1, feature2, resolution=64):
    '''
    feature1, feature2: [1, c, h, w]
    '''
    # upsample
    feature1 = F.interpolate(feature1, size=(resolution, resolution), mode='bilinear', align_corners=False) # [1, c, resolution, resolution]
    feature2 = F.interpolate(feature2, size=(resolution, resolution), mode='bilinear', align_corners=False) # [1, c, resolution, resolution]
    # flatten
    feature1 = rearrange(feature1, '1 c h w -> 1 (h w) c')
    feature2 = rearrange(feature2, '1 c h w -> 1 (h w) c')
